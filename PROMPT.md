...
- 注册到 **root 级常驻浮层 `shell.overlay`**（`order: 100`）：因为**空会话（未开始任务）时产品会隐藏整个会话头部**（`hideChrome = session.blank && composerPhase === 'blank'`），会话级槽（如 `conversation.session.header.utilities`）在空会话里没有渲染位；`shell.overlay` 是全局浮层（list 槽、可叠加、点击穿透由条目自行开启），保证任何状态下右上角都可见。根节点用 `position: fixed; top: 12px; right: 12px`，面板在其下方右对齐展开。
...
- **client 结构**：`lib/client.js` 含 `__ModuleLoader__.load`、`exports.apply`、`exports.inject`（含 `"slots"`）、注册到 root 级浮层 `shell.overlay`（空会话也常驻右上角）。
