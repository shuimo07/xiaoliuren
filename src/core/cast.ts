...
import type { Calendar, CastOptions, CastResult, PalaceResult, RandomCastOptions, Step } from './types.js'
 ...
 */
export function advance(startIndex1based: number, n: number): number {
 ...
 */
export function renderSteps(month: number, day: number, hour: number): Step[] {
 ...
 */
export function compute(month: number, day: number, hour: number): { steps: Step[]; result: PalaceResult } {
  assertPositive(month, '月号')
  assertPositive(day, '日号')
  assertPositive(hour, '时号')
 ...
 */
export function validateNumbers(nums: unknown): number[] {
...
export function castTime(dateInput?: string | Date, opts: CastOptions = {}): CastResult {
...
export function castNumbers(nums: number[], opts: CastOptions = {}): CastResult {
...
export function castRandom(opts: RandomCastOptions = {}): CastResult {
...
function resolveDate(dateInput: string | Date | undefined, now?: Date): Date {
  if (dateInput === undefined) return now ?? new Date()
  if (dateInput instanceof Date) {
    if (Number.isNaN(dateInput.getTime())) throw new Error('无效的日期时间')
    return dateInput
  }
...
function applyLateZiRollover(date: Date, rollover: boolean): Date {
  if (!rollover || date.getHours() < 23) return date
  const next = new Date(date)
  next.setDate(next.getDate() + 1)
  return next
}

function monthDayOf(date: Date, calendar: Calendar): { month: number; day: number } {
  if (calendar === 'lunar') {
...
function buildResult(
  mode: CastResult['mode'],
  input: { month: number; day: number; hour: number; date?: string },
  disclaimer?: string,
): CastResult {
